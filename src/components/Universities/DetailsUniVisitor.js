...
function DetailsUniVisitor() {
  ...
  const fetchProfessorAndRating = async () => {
    try {
      const professorResponse = await fetch(`http://localhost:44364/api/GetUniversityById/${universityId}`);
    ...
    <Container  style={{ width: '75%', marginTop: '20px', padding: '20px' }}>
      ...
      <Grid container spacing={2} marginTop={2}>
        <Grid item xs={12} md={6}>
          <img src={`http://localhost:44364/${university.profilePhotoPath}`} alt={university.name} style={{ width: '300px', maxHeight: '300px', objectFit: 'cover', borderRadius: '8px' }} />
        </Grid>
        <Grid item xs={12} md={6}>
