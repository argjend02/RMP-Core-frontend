...
export default function DashboardAppPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const universityCountResponse = await api.get(
          "/api/AdminDashboard/UniversityCount"
        );
        setUniversityCount(universityCountResponse.data);

        const studentCountResponse = await api.get(
          "/api/AdminDashboard/StudentCount"
        );
        setStudentCount(studentCountResponse.data);

        const departmentCountResponse = await api.get(
          "/api/AdminDashboard/DepartmentCount"
        );
        setDepartmentCount(departmentCountResponse.data);

        const professorCountResponse = await api.get(
          "/api/AdminDashboard/ProfessorCount"
        );
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const universityNamesPromises = Object.keys(universitiesCount).map(
          async (universityId) => {
            ...
            return {
              ...
              studentCount: universitiesCount[universityId],
            };
          }
        );
